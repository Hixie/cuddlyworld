...
procedure TestMechanics1();
   ...
   begin
      ...
      Proxy.ExpectString('Looking out, you see:');
      Proxy.ExpectString('Camp Cuddlyfort');
      Proxy.ExpectString('This is a camp nestled in a forest, under the shadow of a mountain to the north.');
      ...
      Proxy.ExpectString('Which of the balloons do you want to examine, the large pink balloon, the large grey balloon, the large black balloon, the large white balloon, the large violet balloon, the large blue balloon, the large green balloon, the large yellow balloon, the large orange balloon, or the large red balloon? Let''s focus on one at a time.');
...
//         TestWorld.Perform('drop any two balloons', TestPlayer);
      ...
      Proxy.ExpectString('Looking down, you see a ground. The ground is a flat surface of stone. There is an opening there. Beyond that, you can see a flower room.');
      ...
      Proxy.ExpectString('The bag has the name "Tester" embroidered around its rim.');
      Proxy.ExpectString('The bag of holding is on the ground.');
   ...
   begin
      ...
      { setup }
      Zeno := TPlayer.Create('Zeno', '', pHe);
      TestWorld.AddPlayer(Zeno);
      Proxy.SkipEverything();
      TestWorld.Perform('north', TestPlayer);
      TestWorld.Perform('north', Zeno);
      Proxy.StopSkipping();
      ...
      Proxy.ExpectString('You fill the hole with the pile of earth.');
      TestWorld.Perform('dig then push zeno in hole then push dirt in hole', TestPlayer);

      // XXX now test the opposite -- Zeno burying us.
...
begin
   Writeln('MECHANICS I');
   RunMechanicsHarness(@InitTestEden, @RunTest2, {$IFDEF PLAY_IN_TEST_EDEN} True {$ELSE} False {$ENDIF});
   RunMechanicsHarness(@InitTestEden, @RunTest1, {$IFDEF PLAY_IN_TEST_EDEN} True {$ELSE} False {$ENDIF});
end;
